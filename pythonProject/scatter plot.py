...
#             plt.scatter(merged_df[column + '_custom'], merged_df[column + '_standard'])
#             plt.plot([0, 1], [0, 1], color='red', linestyle='--')  # 绘制斜率为1的红色虚线
#             plt.xlabel('Custom Frequency')
#             plt.ylabel('Standard Frequency')
...
#for i, file in enumerate(custom_files[:9]):  # 仅遍历前9个文件
    if file.endswith('.csv'):
        # 提取标题部分
